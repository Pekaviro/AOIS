...
class BinaryCalculator:
    @staticmethod
    def add_binaries(bin1, bin2, max_len):
        result = ''
        carry = 0
        ...
        for i in range(max_len - 1, -1, -1):
            ...
            result = str(bit_result) + result

        # Если есть перенос, добавляем его к результату
        if carry:
            result = '1' + result
    ...
    def sum_additional_code(bin1, bin2):
        ...
        result = BinaryCalculator.add_binaries(bin1, bin2, max_len)
    ...
    def subtract_additional_code(bin1: str, bin2: str) -> str:
        ...
        # Если результат длиннее max_len, обрезаем лишний бит и
        # добавляем его к конечному результату
        if len(result) > max_len:
            result = result[-max_len:]
    ...
    def multiply_direct_code(bin1: str, bin2: str) -> str:
        ...
        for i in range(len(bin2_mod) - 1, -1, -1):
            if bin2_mod[i] == '1':
                # Сдвигаем bin1_mod на (len(bin2_mod) - 1 - i) позиций влево и добавляем к результату
                shifted_bin1 = bin1_mod + '0' * (len(bin2_mod) - 1 - i)
                result = BinaryCalculator.sum_additional_code(result, shifted_bin1)
    ...
    def sum_ieee754(bin1: str, bin2: str) -> str:
        ...
        # Сложение мантисс
        mantissa_sum = BinaryCalculator.sum_additional_code(mantissa1, mantissa2)
